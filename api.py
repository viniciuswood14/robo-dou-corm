...
def parse_mpo_budget_table(full_text_content: str) -> str:
    ...
    # Define MB_UOS localmente, caso a variável global não seja visível neste escopo
    MB_UOS_LOCAL = {
    ...
    for b in blocks:
        ...
        uo_ok = (b["uo_code"] in MB_UOS_LOCAL)
    ...
    for uo_key, lista in grouped.items():
        ...
        if m_code:
            code = m_code.group(1)
            if code in MB_UOS_LOCAL:
                nice_key = f"{code} - {MB_UOS_LOCAL[code]}"
...
def pick_zip_links_from_listing(
    ...
    for a in soup.find_all("a", href=True):
        ...
        if href.lower().endswith(".zip"):
            ...
            filename = href.split("/")[-1].upper() # Ex: 2025-11-21-DO1.ZIP
            
            # Lógica de filtro mais permissiva
            # Verifica se "DO1", "DO2" etc está no nome do arquivo OU no texto do link
            is_match = False
            for sec in wanted:
                # Ex: se sec="DO1", procura "DO1" no filename "2025-11-21-DO1.ZIP"
...
# =====================================================================================
# 2. SERVIR O FRONTEND (STATIC FILES) - FINAL DO ARQUIVO
# =====================================================================================

    ...
    for block in text_blocks:
        ...
        for table in block_soup.find_all('table'):
            for row in table.find_all('tr'):
                ...
                if len(cols) == 10:
                    try:
                        ao, desc, _, _, gnd, _, _, _, _, valor = [norm(c.get_text()) for c in cols]
                        if "PROGRAMÁTICA" in ao.upper() or not valor: continue
...
def parse_public_html_materia(materia_soup: BeautifulSoup, section_str: str) -> Optional[Publicacao]:
    ...
    if not section_num_search: return None
...
async def processar(
    ...
    if source.upper() == "INLABS":
        ...
        try:
            ...
            if not zip_links:
                raise HTTPException(status_code=404, detail=f"Não encontrei ZIPs para a(s) seção(ões) '{', '.join(secs)}' no INLABS.")

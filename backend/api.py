...
# #####################################################################
# ########## VERSÃO 14.1 - Fallback Inteligente (Mantém Tags) ##########
# #####################################################################

app = FastAPI(title="Robô DOU API (INLABS XML) - v14.1 (IA)")
...
class Publicacao(BaseModel): # ... (inalterado) ...
...
class ProcessResponse(BaseModel): # ... (inalterado) ...
    date: str; count: int; publications: List[Publicacao]; whatsapp_text: str

_ws = re.compile(r"\s+")
def norm(s: Optional[str]) -> str: # ... (inalterado) ...
    if not s: return ""
    return _ws.sub(" ", s).strip()

def monta_whatsapp(pubs: List[Publicacao], when: str) -> str: # ... (inalterado) ...
...
def parse_gnd_change_table(full_text_content: str) -> str: # ... (inalterado) ...
    # ... (código igual à v14.0) ...
...
def process_grouped_materia(
    ...
    match = None # Inicializa match (Correção v13.12)
    if not summary:
        match_ementa = re.search(r'EMENTA:(.*?)(Vistos|ACORDAM|\Z)', display_text, re.DOTALL | re.I)
        if match_ementa: summary = norm(match_ementa.group(1))
    is_relevant = False; reason = None; search_content_lower = norm(full_text_content).lower(); clean_text_for_ia = ""
    if "DO1" in section:
        is_mpo = MPO_ORG_STRING in organ.lower()
        if is_mpo:
            found_navy_codes = [code for code in MPO_NAVY_TAGS if code in search_content_lower]
            if found_navy_codes: is_relevant = True; summary_lower = summary.lower();
...
# --- Funções de Rede ---
# ... (inalterado) ...
async def inlabs_login_and_get_session() -> httpx.AsyncClient: # ... (inalterado) ...
    ...
    return client
async def resolve_date_url(client: httpx.AsyncClient, date: str) -> str: # ... (inalterado) ...
...
async def fetch_listing_html(client: httpx.AsyncClient, date: str) -> str: # ... (inalterado) ...
    ...
    return r.text
def pick_zip_links_from_listing(html: str, base_url_for_rel: str, only_sections: List[str]) -> List[str]: # ... (inalterado) ...
...
async def download_zip(client: httpx.AsyncClient, url: str) -> bytes: # ... (inalterado) ...
    r = await client.get(url)
    if r.status_code >= 400: raise HTTPException(status_code=502, detail=f"Falha ao baixar ZIP {url}: HTTP {r.status_code}")
    return r.content
def extract_xml_from_zip(zip_bytes: bytes) -> List[bytes]: # ... (inalterado) ...
...
async def processar_inlabs(# ... (código igual à v13.12) ...
...
async def get_ai_analysis(clean_text: str, model: genai.GenerativeModel) -> Optional[str]: # ... (inalterado) ...
    # ... (código igual à v14.0) ...
...
# === ENDPOINT INTELIGENTE (IA - v14.1) ===
@app.post("/processar-inlabs-ia", response_model=ProcessResponse)
async def processar_inlabs_ia( # ... (inalterado, exceto nome do modelo e Lógica Etapa 5) ...
    data: str = Form(..., description="YYYY-MM-DD"), sections: Optional[str] = Form("DO1,DO2", description="Ex.: 'DO1,DO2,DO3'"), keywords_json: Optional[str] = Form(None)):
    ...
    try: 
        ...
        model_name = "gemini-2.5-pro" # Ou o nome exato que funcionou
        print(f"Inicializando modelo de IA: {model_name}")
    ...
    try:
        ...
        # --- ALTERAÇÃO v14.1 ---
        # 5. Monta o resultado final com Fallback Inteligente
        pubs_finais: List[Publicacao] = []
        for i, p in enumerate(merged_pubs):
            add_pub = True # Assume que a publicação será adicionada
            if i < len(ai_results):
                ai_reason_result = ai_results[i]
                
                if isinstance(ai_reason_result, Exception):
                    # Erro grave na chamada da API
                    p.relevance_reason = f"Erro GRAVE na análise de IA: {ai_reason_result}"
                
                elif ai_reason_result is None:
                    # Fallback (bloqueio de segurança ou erro leve)
                    # Mantém a reason original (do Estágio 1) - Não faz nada aqui
                    pass 
                ...
                elif isinstance(ai_reason_result, str) and "sem impacto direto" in ai_reason_result.lower():
                     # IA funcionou mas indicou irrelevância - MANTÉM a reason original
                     pass 
                
                elif isinstance(ai_reason_result, str): # Se for string e não for erro nem "sem impacto"
                    # IA funcionou e confirmou relevância -> SUBSTITUI a reason
                    p.relevance_reason = ai_reason_result
                
                else:
                    # Caso inesperado, mantém a reason original por segurança
                    print(f"Resultado inesperado da IA: {ai_reason_result}")
                    pass 
...
# --- Endpoint de Teste (v14.0) ---
# ... (inalterado, continua usando o modelo solicitado) ...
@app.get("/test-ia")

...
def parse_gnd_change_table(full_text_content: str) -> str:
    # ##################################################################
    # FUNÇÃO REESCRITA PARA MAIOR ROBUSTEZ
    # ##################################################################
    ...
    for table in soup.find_all('table'):
        for row in table.find_all('tr'):
            ...
            if len(cols) < 5:
                ...
                elif "ACRÉSCIMO" in row_text.upper():
                    current_operation = "acrescimo"
                elif "REDUÇÃO" in row_text.upper() or "CANCELAMENTO" in row_text.upper():
                    current_operation = "reducao"
                continue

            # Processa linha de dados se estivermos em uma seção relevante

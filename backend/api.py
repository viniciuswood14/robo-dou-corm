...
# --- Tentativa de importar parser MB (v14.1) ---
try:
    import mb_portaria_parser
    PARSER_MB_DISPONIVEL = True
    print("✓ Parser MB carregado com sucesso")
except ImportError:
    PARSER_MB_DISPONIVEL = False
    print("⚠ Parser MB não encontrado - funcionalidade desabilitada (não é crítico)")
# -------------------------------------------

# #####################################################################
# ########## VERSÃO 14.1 - (Parser MB Integrado) ####################
# #####################################################################

app = FastAPI(title="Robô DOU API (INLABS XML) - v14.1 (Parser MB Integrado)")
...
# --- Função Helper para Parser MB (v14.1) ---
def tentar_processar_com_parser_mb(zip_bytes: bytes) -> Optional[str]:
    """
    Tenta usar mb_portaria_parser para extrair dados estruturados de portarias MPO.
    Retorna texto formatado ou None se não funcionar.
    
    Esta função é OPCIONAL - se falhar, o sistema continua funcionando normalmente.
    """
    if not PARSER_MB_DISPONIVEL or not zip_bytes:
        return None
    
    try:
        # Cria arquivo temporário com o ZIP
        with tempfile.NamedTemporaryFile(suffix='.zip', delete=False) as tmp:
            tmp.write(zip_bytes)
            tmp_path = tmp.name
        
        # Processa com o parser
        texto_formatado, dados_estruturados = mb_portaria_parser.parse_zip_and_render(tmp_path)
        
        # Remove arquivo temporário
        os.unlink(tmp_path)
        
        # Retorna texto formatado se houver dados
        if dados_estruturados and texto_formatado:
        ...
        print(f"Parser MB falhou (não é crítico): {e}")
        return None
# -------------------------------------------
...
# ### MODIFICAÇÃO v14.1: Adiciona parâmetro zip_bytes para usar parser MB
def process_grouped_materia(
    main_article: BeautifulSoup, 
    full_text_content: str, # Este é o XML/HTML bruto
    custom_keywords: List[str],
    zip_bytes: Optional[bytes] = None  # NOVO: ZIP completo para parser MB
) -> Optional[Publicacao]:
    ...
    if "DO1" in section:
        ...
        if is_mpo:
            ...
            if found_navy_codes:
                ...
                # --- v14.1: TENTATIVA DE USAR PARSER MB ---
                # Tenta parser primeiro APENAS se:
                # 1. É um "navy hit" (tem UGs específicas da Marinha)
                # 2. É um tipo de portaria que o parser sabe processar
                # 3. Temos o ZIP disponível
                parsed_text = None
                if is_mpo_navy_hit_flag and zip_bytes:
                    tipo_parseavel = any([
                        "abre aos orçamentos fiscal" in summary_lower,  # Suplementação
                        "modifica fontes de recursos" in summary_lower,  # Alteração de Fonte
                        "altera parcialmente grupos de natureza de despesa" in summary_lower  # GND
                    ])
                    
                    if tipo_parseavel:
                        parsed_text = tentar_processar_com_parser_mb(zip_bytes)
                
                # Se parser funcionou, usa o resultado
                if parsed_text:
                    reason = parsed_text
                # Senão, usa templates antigos (FALLBACK)
                elif "altera parcialmente grupos de natureza de despesa" in summary_lower:
...
async def processar_inlabs(
    ...
    try:
        ...
        all_xml_blobs = []
        zip_atual = None  # v14.1: Guarda último ZIP processado
        for zurl in zip_links:
            zb = await download_zip(client, zurl)
            zip_atual = zb  # v14.1: Guarda para usar no parser
            ...
            try:
                ...
                if materia_id not in materias:
                    materias[materia_id] = {
                        'main_article': None, 
                        'full_text': '',
                        'zip_bytes': zip_atual  # v14.1: Associa ZIP à matéria
                    }
        ...
        for materia_id, content in materias.items():
            if content['main_article']:
                publication = process_grouped_materia(
                    ...
                    content['full_text'], 
                    custom_keywords,
                    zip_bytes=content.get('zip_bytes')  # v14.1: Passa ZIP para parser
                )

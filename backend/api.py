...
# ####################################################################
# ########## VERSÃO 10.3 - FILTRO INTELIGENTE DE PESSOAL ##########
# ####################################################################

app = FastAPI(title="Robô DOU API (INLABS XML) - v10.3 Filtro Inteligente")
...
PERSONNEL_ACTION_VERBS = [
    "nomear", "designar", "exonerar", "dispensar", "promover", "promovido",
...
TERMS_AND_ACRONYMS_S2 = ["SOF", "SG-MD", "SEORI-MD", "DEORG-MD", "DEOFM", "VAlte (IM)", "CAlte (IM)", "CMG (IM)", "CF (IM)", "CC (IM)", "CT (IM)", "1T (IM)", "2ºT (IM)"]
...
def process_grouped_materia(main_article: BeautifulSoup, full_text_content: str) -> Optional[Publicacao]:
    ...
    if "DO1" in section:
        ...
        for term in TERMS_AND_ACRONYMS_S2:
            if re.search(r'\b' + re.escape(term.lower()) + r'\b', search_content_lower):
                is_relevant = True

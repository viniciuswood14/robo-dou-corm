...
# #######################################################################
# ########## VERSÃO 12.2 - EXTRATOR DE DADOS MAIS ROBUSTO ##########
# #######################################################################

app = FastAPI(title="Robô DOU API (INLABS XML) - v12.2 Extrator Robusto")
...
def parse_gnd_change_table(full_text_content: str) -> str:
    ...
    for table in soup.find_all('table'):
        for row in table.find_all('tr'):
            cols = [norm(c.get_text()) for c in row.find_all('td')]
            
            # Lógica para identificar cabeçalhos de seção (Unidade, Operação)
            if len(cols) < 5: # Linhas de cabeçalho geralmente têm poucas colunas
                row_text = " ".join(cols).upper()
                if "UNIDADE:" in row_text:
                    match = re.search(r'UNIDADE:\s*(.*)', " ".join(c.get_text(" ", strip=True) for c in row.find_all('td')))
                    if match:
            ...
            if current_unidade and any(tag in current_unidade for tag in MPO_NAVY_TAGS.keys()):
                try:
                    ao = cols[0]
                    desc = cols[1]
                    gnd = cols[4].replace('-','').replace('ODC','').replace('INV','')
                    valor = cols[9]

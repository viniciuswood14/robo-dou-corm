...
# #####################################################################
# ########## VERSÃO 13.11 - Usando Modelo Indicado pelo Usuário ##########
# #####################################################################

app = FastAPI(title="Robô DOU API (INLABS XML) - v13.11 (IA)")
        ...
        for row in table.find_all('tr'):
            ...
            if current_unidade and current_operation and any(tag in current_unidade for tag in MPO_NAVY_TAGS.keys()):
                try:
                    ...
                    clean_gnd = gnd.replace('-','').replace('ODC','').replace('INV',''); line = f"- AO {ao} - {desc} | GND: {clean_gnd} | Valor: {valor}"
...
def process_grouped_materia(
    ...
    if not summary: match = re.search(r'EMENTA:(.*?)(Vistos|ACORDAM)', display_text, re.DOTALL | re.I); 
        ...
        if not is_relevant:
            for name in NAMES_TO_TRACK:
                ...
                for match in re.finditer(name_lower, clean_search_content_lower):
                    start_pos = max(0, match.start() - 150); context_window_text = clean_search_content_lower[start_pos:match.start()]
...
async def processar_inlabs(
    ...
    try:
        listing_url = await resolve_date_url(client, data); html = await fetch_listing_html(client, data)
        ...
        for blob in all_xml_blobs:
            try:
                soup = BeautifulSoup(blob, 'lxml-xml'); article = soup.find('article')
...
async def get_ai_analysis(clean_text: str, model: genai.GenerativeModel) -> Optional[str]:
    ...
    try:
        ...
        response = await model.generate_content_async(prompt) # Chamada Simplificada
        try:
            ...
            if analysis: return analysis
                ...
                print(f"Resposta da IA vazia. Razão: {finish_reason}"); return None # Fallback
        ...
        print(f"Erro na API do Gemini: {e}"); error_msg = str(e).lower()
...
async def processar_inlabs_ia(
    ...
    try:
        ...
        # ATENÇÃO: Verifique se "gemini-2.5-pro" é o ID exato.
        model_name = "gemini-2.5-pro" 
        print(f"Inicializando modelo de IA: {model_name}") # Log para confirmação
    ...
    # ... (Estágio 1 - código inalterado da v13.9) ...
    secs = [s.strip().upper() for s in sections.split(",") if s.strip()] if sections else ["DO1"]
    custom_keywords = []
    if keywords_json:
        try:
    ...
    try:
        ...
        # 5. Monta o resultado final (Lógica v13.7 inalterada)
        ...
        for i, p in enumerate(merged_pubs):
            if i < len(ai_results):
                ...
                # Descarte: "Sem impacto direto" ou tipo inesperado
...
# --- Endpoint de Teste (v13.11 - Modelo Solicitado) ---
@app.get("/test-ia")
async def test_ia_endpoint():
    # ... (código quase inalterado da v13.9, apenas o nome do modelo) ...
    if not GEMINI_API_KEY: raise HTTPException(status_code=500, detail="GEMINI_API_KEY não configurada.")
    try: 
        # --- ALTERAÇÃO v13.11 ---
        model_name = "gemini-2.5-pro" # Usando o modelo solicitado
        print(f"Inicializando modelo de teste: {model_name}")
        model = genai.GenerativeModel(model_name) 
